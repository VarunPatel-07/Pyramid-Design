                  ...
                  {Array.isArray(selectedValue) && (
                    <div className="flex items-center justify-start flex-wrap gap-1">
                      {selectedValue?.map((data: string, index: number) => (
                        <span
                          className="text-black font-inter capitalize text-nowrap text-ellipsis overflow-hidden px-2 py-1 bg-gray-200 text-xs rounded-sm flex items-center justify-start gap-1.5"
                          key={index}>
        ...
        {isOpen && (
          <div
            ...
            <div
              ...
              <ul
                ref={listRef}
                className="max-h-32.5 h-full overflow-auto w-full bg-gray-100 shadow-md rounded-lg hide-scrollbar">
                {!loading ? (
